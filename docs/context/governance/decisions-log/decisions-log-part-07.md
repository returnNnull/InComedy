...
- Date: 2026-03-25
- Status: accepted
- Decision: `EPIC-070` должен стартовать provider-agnostic/manual-settlement-ready backend foundation: donation intent можно создавать только для комика с `verified` payout profile, а первый delivery шаг не должен выбирать внешний donation/payout provider, переиспользовать ticketing PSP adapter как готовое решение или включать checkout/webhook/payout automation.
- Rationale: Пользователь явно потребовал начать `EPIC-070`, но никакой donation/payout provider отдельно не подтверждался. Product/compliance docs уже требуют держать donations отдельным доменом, не смешивать их с ticket checkout и обеспечивать verified payout identity до включения flow. Без зафиксированного решения backend slice легко мог бы случайно закодировать существующий ticketing PSP как implicit provider choice и создать ложное впечатление rollout-ready payments surface.
- Consequences: Первый bounded step `TASK-087` добавляет только provider-agnostic `:domain:donations`, manual-settlement-ready persistence, protected payout-profile routes и donation intent create/list semantics с `checkout_available=false`; `risk-log.md` должен сохранить открытым compliance/provider risk `R-005`, а следующие `TASK-088`/`TASK-089` обязаны продолжать foundation without selecting PSP до отдельного user confirmation.
