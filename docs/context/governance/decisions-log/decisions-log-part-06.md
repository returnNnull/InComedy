...
- Date: 2026-03-25
- Status: accepted
- Decision: При возникновении blocker-ов исполнитель должен сначала проверить `docs/context/engineering/issue-resolution-log.md` на уже известные симптомы и repair path; для iOS simulator/Xcode destination проблем первого типа приоритетным первым действием считать запуск Xcode или его перезапуск, если приложение зависло.
- Rationale: Пользователь явно потребовал прекратить повторные циклы “новой” диагностики там, где решение уже задокументировано. Для текущей симптоматики `CoreSimulatorService` / `showdestinations` это означает, что не нужно сразу уходить в глубокий simulator diagnostics loop: сначала надо проверить накопленную запись `I-001`, а в её рамках первым repair step считать запущен ли Xcode и не завис ли он. Это снижает количество повторяющихся отчётов и ускоряет восстановление среды.
- Consequences: `automation-executor-prompt.md`, `context-protocol.md`, `docs/context/README.md`, `docs/README.md`, `00-current-state.md`, `issue-resolution-log` и task/session memory должны явно требовать pre-check существующих записей в issue log до новой диагностики blocker-а; запись `I-001` должна содержать шаг “запустить Xcode или перезапустить его, если завис” как первый repair action для этой симптоматики.

## D-077

- Date: 2026-03-25
- Status: accepted
- Decision: Операционные docs должны разделять bootstrap snapshot, recovery checkpoint, executor checklist, executor policy, delivery status, verification memory и next-epic queue по отдельным файлам, чтобы новый запуск не читал лишний исторический контекст.
- Rationale: Пользователь явно потребовал повысить эффективность через снижение перегруза в ключевых context docs. Прежняя структура смешивала несколько ролей в одном файле: runbook + policy, architecture + delivery status, test strategy + verification history, current-state + extended explanation, active-run + historical diagnostics. Это увеличивало стоимость каждого bootstrap/read-only/docs-only прохода.
- Consequences: `automation-executor-prompt.md` остаётся короткой входной точкой и делегирует на `executor-checklist.md` и `executor-policy.md`; `00-current-state.md` должен оставаться компактным snapshot; `active-run.md` — коротким recovery checkpoint; `test-strategy.md` и `architecture-overview.md` не должны хранить run-history/delivery-history; для этого используются `verification-memory.md` и `implementation-status.md`; очередь следующего epic живёт в `product/next-epic-queue.md`.
