...
- Перед новой диагностикой blocker-а сначала проверь `docs/context/engineering/issue-resolution-log.md`, нет ли там уже существующей записи с теми же симптомами и известным repair path.
- Для iOS simulator / Xcode destination симптомов уровня `CoreSimulatorService`, `showdestinations`, placeholder destinations и похожих первым repair step считай запуск Xcode; если Xcode уже открыт, но завис или не отвечает, перезапусти его, и только потом переходи к более глубокой диагностике.
