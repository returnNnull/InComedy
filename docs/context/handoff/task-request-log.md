...
- Ordered plan для `EPIC-069` зафиксирован в `task-request-template/task-request-template-part-36.md`:
  ...
  - `TASK-085` — shared/data realtime subscription contract для lineup live updates. Status: `in_progress` и является текущим active recovery checkpoint.
  - `TASK-086` — Android/iOS realtime wiring и executable verification. Status: `planned`.
- Текущий active recovery posture для `EPIC-069`: локальная commit boundary `TASK-084` закрыта commit-ом `ecb5b96`, active recovery переключён на `TASK-085/in_progress`.
