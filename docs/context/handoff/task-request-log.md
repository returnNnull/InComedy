...
- Ordered plan для `EPIC-069` зафиксирован в `task-request-template/task-request-template-part-36.md`:
  ...
  - `TASK-085` — shared/data realtime subscription contract для lineup live updates. Status: `planned` и является следующим продуктовым bounded step после локального commit для `TASK-084`.
  - `TASK-086` — Android/iOS realtime wiring и executable verification. Status: `planned`.
- Текущий active recovery posture для `EPIC-069`: `TASK-084` в статусе `ready_to_commit` до закрытия локальной commit boundary; только после этого recovery может переключиться на `TASK-085`.
...
- `TASK-084` is now `completed`: backend realtime kickoff для `EPIC-069` доставил public `/ws/events/{eventId}` live channel с initial snapshot, audience-safe `lineup.changed` / `stage.current_changed` payload-ами, server-local in-memory broadcaster и fanout из approve/reorder/live-state mutation path-ов. Targeted server verification green; текущий recovery gate — локальный commit для `TASK-084`, после которого следующим bounded продуктовым шагом станет `TASK-085`. The recorded security verdict for this realtime backend slice is `published-public-event gating + peer rate limiting + audience-safe payload only`.
