...
- Android lineup management tab now also covers live-stage summary (`current performer` / `next up`) and organizer live control callbacks in `composeApp/src/test/kotlin/com/bam/incomedy/feature/lineup/ui/LineupManagementTabContentTest.kt`.
...
- `testLineupTabShowsApplicationsAndReorderSurface` now also asserts live-stage summary and organizer live-control affordances over the shared lineup fixture. The repo-side Xcode bridge now bootstrap-s Kotlin/Native into `.gradle/xcode/konan-bootstrap`, and the remaining SwiftUI `#Preview` macro blocks were replaced with `PreviewProvider` fallbacks, so generic `xcodebuild ... build` is now green in this sandbox. `TASK-073` still cannot be marked `completed` here because `xcodebuild -project iosApp/iosApp.xcodeproj -scheme iosAppUITests -showdestinations` continues to lose `CoreSimulatorService`, disables simulator device support, and exposes only placeholder destinations (`My Mac`, `Any iOS Device`, `Any iOS Simulator Device`). Re-run the targeted iOS verification on an unrestricted environment before marking `TASK-073` completed.
...
- Live-stage shared/data/feature lineup integration now also relies on `:feature:lineup:allTests :data:lineup:compileKotlinMetadata :shared:compileKotlinMetadata :composeApp:compileDebugKotlin`; `LineupViewModelTest` now covers organizer-side `up_next` / `on_stage` read and mutation semantics inside the shared feature layer.
