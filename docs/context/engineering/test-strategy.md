...
- Event/EventHallSnapshot backend changes must cover route behavior, snapshot freeze invariants, and migration-path verification for the organizer event slice.
...
- iOS organizer event tab is covered by `iosApp/iosAppUITests/iosAppUITests.swift` through `testEventTabShowsEventManagementSurface`, which smoke-checks tab reachability plus stable workspace/venue/template selectors, create controls, and publish action over the shared event fixture.
...
- Event foundation automation now also includes `:domain:event:jvmTest`, `:composeApp:testDebugUnitTest`, `:server:test --tests 'com.bam.incomedy.server.db.DatabaseMigrationRunnerTest' --tests 'com.bam.incomedy.server.events.EventRoutesTest'`, `xcodebuild -project iosApp/iosApp.xcodeproj -scheme iosApp -configuration Debug -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' build CODE_SIGNING_ALLOWED=NO`, and the targeted event XCUITest run for the new iOS tab.
