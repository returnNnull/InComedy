...
- Event/EventHallSnapshot backend changes must cover route behavior, detail/update override validation, snapshot freeze invariants, and migration-path verification for the organizer event slice.
...
- iOS organizer event tab is covered by `iosApp/iosAppUITests/iosAppUITests.swift` through `testEventTabShowsEventManagementSurface`, which smoke-checks tab reachability plus stable workspace/venue/template selectors, create controls, publish action, and override-editor surface over the shared event fixture.
