...
- Partial:
  ...
  - comedian applications foundation now includes backend migration/persistence plus authenticated routes for comedian submit and organizer owner/manager review (`submit/list/status change`) with structured diagnostics and regression coverage, but lineup entry creation, explicit `order_index`, reorder/live-stage semantics, and shared/mobile surfaces are still missing;
...
- Planned next bounded contexts:
  - lineup ordering + approved-application -> lineup entry bridge,
  - donations/payouts,
  - notifications,
  - analytics.
